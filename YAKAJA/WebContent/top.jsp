...
<body>
 ...
 <c:if test="${not empty sessionScope.YakajaVO}">
    <c:if test="${sessionScope.YakajaVO.memAdmin == 0}">
        <a href="myinfo.do" class="nav-item nav-link">내정보</a>
        <a href="resv.do" class="nav-item nav-link">예약정보확인</a>
    </c:if>
        <c:if test="${sessionScope.YakajaVO.memAdmin == 1}">
        <a href="hotelJoin.jsp" class="nav-item nav-link">호텔등록</a>
        <a href="myinfo.do" class="nav-item nav-link">내정보</a>
        <a href="index.jsp" class="nav-item nav-link">예약목록확인</a>
    </c:if>
    
       <!--<a href="contact.html" class="nav-item nav-link">나의정보</a>-->
            
            </c:if>
            <c:if test="${empty sessionScope.YakajaVO}"></c:if>
            ...
            <c:if test="${not empty sessionScope.YakajaVO}">
            	<a href="login.jsp" class="btn btn-primary py-4 px-lg-5 d-none d-lg-block">로그아웃<i class="fa fa-arrow-right ms-3"></i></a>
        	</c:if>
        </div>
    </nav>
    <!-- Navbar End -->
